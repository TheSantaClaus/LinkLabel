...
open class LinkLabel: UILabel, UIGestureRecognizerDelegate {
    ...
    override public init(frame: CGRect) {
        linkTextAttributes = [
            NSAttributedString.Key.underlineStyle.rawValue: NSNumber(value: NSUnderlineStyle.single.rawValue as Int)]
    ...
    fileprivate func setupAttributes() {
        ...
        for attribute in self.standardTextAttributes {
            mutableAttributedText.addAttribute(NSAttributedString.Key(rawValue: attribute.attributeName), value: attribute.value, range: attribute.range)
        }
        ...
        for linkAttribute in self.linkAttributes {
            if linkAttribute === self.highlightedLinkAttribute {
                for (attributeName, value): (String, AnyObject) in self.highlightedLinkTextAttributes {
                    mutableAttributedText.addAttribute(NSAttributedString.Key(rawValue: attributeName), value: value, range: linkAttribute.range)
                }
            } else {
